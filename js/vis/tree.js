...
$('.svg-canvas').hide();
/**
 * Loads the visualization using the provided JSON String
...
function createVisualization(jsonData){
  ...
  d3.json(jsonData, function(err, dataset) {
    var textLength = {};
    ...
    textHolder.selectAll("texts")
              ...
              .text(function(d){
                ...
                return d.name + " : " + d.textLength + " characters compared.";
              })
    ...
    nodeHolder.selectAll("rect")
              ...
              .on('mouseover', function(d){
                d3.select(this).attr('width', function(d){return getNodeWidth(d.l, d.group) * 2;});
                var links = getLinksByNode(dataset, d.id);
                links.forEach(function(link, index){
                  
                });
              })
              .on('mouseout', function(d){
                d3.select(this).attr('width', function(d){return getNodeWidth(d.l, d.group);});
              })
    ...
    linkHolder.selectAll('path')
      ...
      **/
      ...
      /**
       * Returns all the links this node is a part of
       */
      function getLinksByNode(dataset, nodeName){
        var i = 0; var temp;
        var results = [];
        do{
          temp = dataset.links[i];
          if(temp.source == nodeName || temp.target == nodeName){
            results.push(temp);
          }
          i++;
        }while(i < dataset.links.length)
        return results;
      }
      /**
